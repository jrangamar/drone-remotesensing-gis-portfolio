...
TIN_BASE_PATH = PROJECT_ROOT / "data/processed/p02_estimated_tin_base.tif"
HEIGHT_OUTPUT_PATH = (
    PROJECT_ROOT / "data/processed/p02_stockpile_height_above_tin.tif"
)
POSITIVE_OUTPUT_PATH = (
    PROJECT_ROOT / "data/processed/p02_stockpile_height_positive.tif"
)
METRICS_OUTPUT_PATH = PROJECT_ROOT / "data/results/p02_stockpile_metrics.csv"
...
def main() -> None:
    ...
    if not TIN_BASE_PATH.exists():
        raise FileNotFoundError(f"TIN base not found: {TIN_BASE_PATH}")

    with rasterio.open(TIN_BASE_PATH) as tin_src:
        tin = tin_src.read(1).astype("float64")
        tin_profile = tin_src.profile.copy()
        tin_transform = tin_src.transform
        tin_crs = tin_src.crs
        tin_nodata = tin_src.nodata

        tin_valid = np.isfinite(tin)
        if tin_nodata is not None:
            tin_valid &= tin != tin_nodata

        dsm_aligned = np.full(tin.shape, np.nan, dtype="float64")
        ...
        with rasterio.open(DSM_PATH) as dsm_src:
            reproject(
                ...
                dst_transform=tin_transform,
                dst_crs=tin_crs,
                dst_nodata=np.nan,
                resampling=Resampling.bilinear,
            )

    valid = tin_valid & np.isfinite(dsm_aligned)

    if not np.any(valid):
        raise RuntimeError("No overlapping valid DSM and TIN pixels were found.")

    height = np.full(tin.shape, OUTPUT_NODATA, dtype="float64")
    height[valid] = dsm_aligned[valid] - tin[valid]
    ...
    positive_height = np.full(tin.shape, OUTPUT_NODATA, dtype="float64")
    positive_height[valid] = np.maximum(height[valid], 0.0)

    positive_values = positive_height[valid]

    pixel_width = abs(tin_transform.a)
    pixel_height = abs(tin_transform.e)
    ...
    mean_positive_height = float(np.mean(positive_values[positive_values > 0]))
    maximum_height = float(np.max(positive_values))

    write_raster(HEIGHT_OUTPUT_PATH, height, tin_profile)
    write_raster(POSITIVE_OUTPUT_PATH, positive_height, tin_profile)

    metrics = pd.DataFrame(
        [
            {
                "pile_id": "P02",
                "base_model": "perimeter_tin",
